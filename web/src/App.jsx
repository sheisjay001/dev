      ...
      <header className="bg-white/80 backdrop-blur sticky top-0 z-30 border-b border-gray-100 shadow-sm flex-none">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <nav className="flex space-x-1 bg-gray-100/50 p-1 rounded-lg">
              <button onClick={()=>setTab('dashboard')} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${tab==='dashboard'?'bg-white text-brand-700 shadow-sm ring-1 ring-black/5':'text-gray-500 hover:text-gray-900'}`}>Dashboard</button>
              <button onClick={()=>setTab('contacts')} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${tab==='contacts'?'bg-white text-brand-700 shadow-sm ring-1 ring-black/5':'text-gray-500 hover:text-gray-900'}`}>Contacts</button>
              <button onClick={()=>setTab('deals')} className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${tab==='deals'?'bg-white text-brand-700 shadow-sm ring-1 ring-black/5':'text-gray-500 hover:text-gray-900'}`}>Deals</button>
            </nav>
